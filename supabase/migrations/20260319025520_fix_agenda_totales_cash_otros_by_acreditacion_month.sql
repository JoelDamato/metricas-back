-- Ajuste solicitado:
-- cash_collected_otros_meses debe tomar el mes visualizado por F.acreditacion,
-- sumando cash_collected_total de comprobantes acreditados en ese mes
-- cuya agenda (f_venta) NO fue en ese mismo mes.
...
cash_agenda_agg AS (
  ...
  WHERE acreditacion_anio IS NOT NULL
    ...
    AND acreditacion_anio = venta_anio
    AND acreditacion_mes = venta_mes
  GROUP BY acreditacion_anio, acreditacion_mes, origen_key, estrategia_key
),
cash_otros_agg AS (
  SELECT
  ...
  WHERE acreditacion_anio IS NOT NULL
    AND acreditacion_mes IS NOT NULL
    AND (acreditacion_anio <> venta_anio OR acreditacion_mes <> venta_mes)
  GROUP BY acreditacion_anio, acreditacion_mes, origen_key, estrategia_key
),
combos AS (
