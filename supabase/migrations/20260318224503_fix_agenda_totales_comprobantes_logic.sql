-- Ajuste de agenda_totales:
-- Facturacion y cash collected pasan a calcularse desde comprobantes
-- por fecha de venta (f_venta), filtrando Tipo=Venta y producto no club.
-- Se mantiene el resto de métricas desde la vista base original.

DO $$
BEGIN
  IF to_regclass('public.agenda_totales_base') IS NULL THEN
    EXECUTE 'ALTER VIEW public.agenda_totales RENAME TO agenda_totales_base';
  END IF;
END $$;
...
WITH comprobantes_filtrados AS (
  SELECT
    ...
    COALESCE(NULLIF(BTRIM(c.origen), ''), 'Sin origen') AS origen,
