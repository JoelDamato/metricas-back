...
WITH fecha_actual AS (
  ...
  SELECT
    ...
    CASE WHEN c.fecha_de_agendamiento IS NULL THEN NULL::integer ELSE EXTRACT(YEAR FROM c.fecha_de_agendamiento)::integer END AS agenda_anio,
    CASE WHEN c.fecha_de_agendamiento IS NULL THEN NULL::integer ELSE EXTRACT(MONTH FROM c.fecha_de_agendamiento)::integer END AS agenda_mes,
...
SELECT
  ...
  COALESCE(
    b.aov,
    CASE
      WHEN COALESCE(v.total_ventas_comp, 0::bigint) > 0 THEN COALESCE(f.facturacion_total_mes, 0::numeric) / v.total_ventas_comp::numeric
      ELSE 0::numeric
    END
  ) AS aov,
...
LEFT JOIN base_norm b
  ON b.anio = combos.anio AND b.mes = combos.mes AND b.origen_key = combos.origen_key AND b.estrategia_key = combos.estrategia_key
LEFT JOIN fact_agg f
  ON f.anio = combos.anio AND f.mes = combos.mes AND f.origen_key = combos.origen_key AND f.estrategia_key = combos.estrategia_key
LEFT JOIN ventas_agg v
  ON v.anio = combos.anio AND v.mes = combos.mes AND v.origen_key = combos.origen_key AND v.estrategia_key = combos.estrategia_key
LEFT JOIN cash_real_agg cr
  ON cr.anio = combos.anio AND cr.mes = combos.mes AND cr.origen_key = combos.origen_key AND cr.estrategia_key = combos.estrategia_key
LEFT JOIN cash_agenda_agg ca
  ON ca.anio = combos.anio AND ca.mes = combos.mes AND ca.origen_key = combos.origen_key AND ca.estrategia_key = combos.estrategia_key
LEFT JOIN cash_otros_agg co
  ON co.anio = combos.anio AND co.mes = combos.mes AND co.origen_key = combos.origen_key AND co.estrategia_key = combos.estrategia_key;
