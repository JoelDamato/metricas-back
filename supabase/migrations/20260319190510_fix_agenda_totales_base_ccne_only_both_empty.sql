 ...
 SELECT
    ...
    max(COALESCE(p.total_paid_upfront, 0::numeric)) AS total_paid_upfront,
        ...
        CASE
            ...
            ELSE 0::numeric
        END AS tasa_cierre,
    count(*) FILTER (WHERE l.agendo = 'Agendo'::text AND l.aplica = 'Aplica'::text AND (l.call_confirm IS NULL OR btrim(l.call_confirm) = ''::text) AND (l.cc_whatsapp IS NULL OR btrim(l.cc_whatsapp) = ''::text)) AS ccne,
    count(*) FILTER (WHERE l.agendo = 'Agendo'::text AND l.aplica = 'Aplica'::text AND (l.call_confirm IS NULL OR btrim(l.call_confirm) = ''::text) AND (l.cc_whatsapp IS NULL OR btrim(l.cc_whatsapp) = ''::text) AND l.llamada_meg = 'Efectuada'::text) AS ccne_efectuadas,
