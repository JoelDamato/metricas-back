...
with lead_source as (
  select
    l.*,
    coalesce(nullif(btrim(l.origen_actual), ''), l.origen) as marketing_origin
...
lead_norm as (
  select
    ...
    case
      ...
      else coalesce(l.marketing_origin, 'Sin origen')
    end as origen_norm
  from lead_source l
...
comp_source as (
  select
    c.*,
    coalesce(nullif(btrim(lo.marketing_origin), ''), c.origen) as marketing_origin
...
comp_norm as (
  select
    ...
    case
      ...
      else coalesce(c.marketing_origin, 'Sin origen')
    end as origen_norm
  from comp_source c
),
fechas as (
