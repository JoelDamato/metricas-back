-- Incluye combinaciones (anio,mes,origen,estrategia) presentes en comprobantes
-- aunque no existan en agenda_totales_base, para que facturacion/cash no queden afuera.
...
comprobantes_agg AS (
  SELECT
    ...
    origen,
    estrategia_a,
    COUNT(*)::int AS total_ventas_comp,
...
combos_faltantes AS (
  SELECT
    ...
    ca.anio,
    ca.mes,
    0::bigint AS total_agendados,
    0::bigint AS total_aplica,
    0::bigint AS total_respondio,
    0::bigint AS total_confirmo,
    0::bigint AS total_cancelado,
    0::bigint AS total_no_asistidas,
    0::bigint AS total_pendientes,
    0::bigint AS total_efectuadas,
    ca.total_ventas_comp::bigint AS total_ventas,
    0::numeric AS total_paid_upfront,
    0::numeric AS aov,
    0::numeric AS tasa_cierre,
    0::bigint AS ccne,
    0::bigint AS ccne_efectuadas,
    0::bigint AS ccne_vendidas,
    0::bigint AS cce,
    0::bigint AS cce_efectuadas,
    0::bigint AS cce_vendidas,
    ca.facturacion_total_mes,
    ca.cash_collected_real_mes,
    ca.cash_collected_otros_meses,
    ca.cash_collected_agendas_mes,
    0::bigint AS meg_1,
    0::bigint AS meg_2,
    0::bigint AS personalizado,
    0::bigint AS renovacion_meg_personalizado,
    0::bigint AS reportes_financieros,
    0::bigint AS renovacion_meg,
    0::bigint AS renovacion_meg_2,
    0::bigint AS meg_2_1,
    0::bigint AS renovacion_meg_2_1,
    ca.facturacion_total_mes AS facturacion_f_agenda,
    ca.cash_collected_agendas_mes AS cash_collected_agenda
  FROM comprobantes_agg ca
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.agenda_totales_base b
    WHERE b.anio = ca.anio
      AND b.mes = ca.mes
      AND COALESCE(NULLIF(BTRIM(b.origen), ''), 'Sin origen') = ca.origen
      AND COALESCE(NULLIF(BTRIM(b.estrategia_a), ''), 'Sin estrategia') = ca.estrategia_a
  )
)
SELECT * FROM base_con_montos
UNION ALL
SELECT * FROM combos_faltantes;
