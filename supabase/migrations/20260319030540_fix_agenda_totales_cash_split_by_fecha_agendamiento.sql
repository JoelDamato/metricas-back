-- Regla ajustada por negocio:
-- - Mes visualizado se define por F.acreditacion.
-- - F.venta debe pertenecer al mismo mes visualizado.
-- - cash_collected_agendas_mes: agenda en el mismo mes (fecha_de_agendamiento)
-- - cash_collected_otros_meses: agenda en otro mes
-- De esta forma: CC Real del mes = CC agendas del mes + CC de otros meses.
...
comprobantes_base AS (
  SELECT
    ...
    CASE WHEN c.fecha_de_agendamiento IS NULL THEN NULL ELSE EXTRACT(YEAR FROM c.fecha_de_agendamiento)::int END AS agenda_anio,
    CASE WHEN c.fecha_de_agendamiento IS NULL THEN NULL ELSE EXTRACT(MONTH FROM c.fecha_de_agendamiento)::int END AS agenda_mes,
...
-- CC Real del mes por fecha de acreditacion, pero solo ventas del mismo mes
cash_real_agg AS (
  SELECT
  ...
  WHERE acreditacion_anio IS NOT NULL
    AND acreditacion_mes IS NOT NULL
    AND venta_anio = acreditacion_anio
    AND venta_mes = acreditacion_mes
  GROUP BY acreditacion_anio, acreditacion_mes, origen_key, estrategia_key
),
-- CC de agendas del mes: fecha de agenda en el mismo mes
cash_agenda_agg AS (
  SELECT
    acreditacion_anio AS anio,
    acreditacion_mes AS mes,
    origen_key,
    estrategia_key,
    SUM(cash_collected_total)::numeric AS cash_collected_agendas_mes
  FROM comprobantes_base
  WHERE acreditacion_anio IS NOT NULL
    AND acreditacion_mes IS NOT NULL
    AND venta_anio = acreditacion_anio
    AND venta_mes = acreditacion_mes
    AND agenda_anio = acreditacion_anio
    AND agenda_mes = acreditacion_mes
  GROUP BY acreditacion_anio, acreditacion_mes, origen_key, estrategia_key
),
-- CC de otros meses: agenda en otro mes
cash_otros_agg AS (
  SELECT
    acreditacion_anio AS anio,
    acreditacion_mes AS mes,
    origen_key,
    estrategia_key,
    SUM(cash_collected_total)::numeric AS cash_collected_otros_meses
  FROM comprobantes_base
  WHERE acreditacion_anio IS NOT NULL
    AND acreditacion_mes IS NOT NULL
    AND venta_anio = acreditacion_anio
...
SELECT
  ...
  COALESCE(
    b.aov,
    CASE WHEN COALESCE(f.total_ventas_comp, 0) > 0
