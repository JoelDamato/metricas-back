-- Evita duplicados en agenda_detalle_por_origen_closer al consolidar base por clave
-- antes de mezclar con comprobantes.
...
base_agg AS (
  SELECT
    ...
    origen_key,
    closer_key,
    SUM(COALESCE(total_leads, 0))::bigint AS total_leads,
    SUM(COALESCE(total_aplica, 0))::bigint AS total_aplica,
    SUM(COALESCE(total_efectuadas, 0))::bigint AS total_efectuadas,
    SUM(COALESCE(total_ventas, 0))::bigint AS total_ventas,
    SUM(COALESCE(total_paid_upfront, 0))::numeric AS total_paid_upfront,
    SUM(COALESCE(ccne, 0))::bigint AS ccne,
    SUM(COALESCE(ccne_efectuadas, 0))::bigint AS ccne_efectuadas,
    SUM(COALESCE(ccne_vendidas, 0))::bigint AS ccne_vendidas,
    SUM(COALESCE(cce, 0))::bigint AS cce,
    SUM(COALESCE(cce_efectuadas, 0))::bigint AS cce_efectuadas,
    SUM(COALESCE(cce_vendidas, 0))::bigint AS cce_vendidas,
    SUM(COALESCE(meg_1, 0))::bigint AS meg_1,
    SUM(COALESCE(meg_2, 0))::bigint AS meg_2,
    SUM(COALESCE(personalizado, 0))::bigint AS personalizado,
    SUM(COALESCE(reportes_financieros, 0))::bigint AS reportes_financieros,
    SUM(COALESCE(renovacion_meg, 0))::bigint AS renovacion_meg,
    SUM(COALESCE(renovacion_meg_2, 0))::bigint AS renovacion_meg_2,
    SUM(COALESCE(meg_2_1, 0))::bigint AS meg_2_1,
    SUM(COALESCE(renovacion_meg_2_1, 0))::bigint AS renovacion_meg_2_1,
    SUM(COALESCE(renovacion_meg_personalizado, 0))::bigint AS renovacion_meg_personalizado,
    SUM(COALESCE(total_respondio, 0))::bigint AS total_respondio,
    SUM(COALESCE(total_confirmo, 0))::bigint AS total_confirmo,
    SUM(COALESCE(total_cancelado, 0))::bigint AS total_cancelado,
    SUM(COALESCE(total_no_asistidas, 0))::bigint AS total_no_asistidas,
    SUM(COALESCE(total_pendientes, 0))::bigint AS total_pendientes
  FROM base_norm
...
comprobantes_base AS (
  SELECT
    ...
    CASE WHEN c.f_acreditacion IS NULL THEN NULL ELSE EXTRACT(YEAR FROM c.f_acreditacion)::int END AS acreditacion_anio,
    CASE WHEN c.f_acreditacion IS NULL THEN NULL ELSE EXTRACT(MONTH FROM c.f_acreditacion)::int END AS acreditacion_mes,
    CASE
...
SELECT
  ...
  CASE
    WHEN COALESCE(b.total_aplica, 0) > 0
    THEN COALESCE(f.total_ventas_comp, b.total_ventas, 0)::numeric / b.total_aplica::numeric
    ELSE 0::numeric
  END AS tasa_cierre,
  CASE
