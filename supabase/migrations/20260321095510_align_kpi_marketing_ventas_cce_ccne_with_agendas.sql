...
select
  ...
  case
    ...
    else coalesce(vtas.ventas_totales, 0::numeric) /
      (
        count(l.*) filter (
          where l.agendo = 'Agendo'
            and l.aplica = 'Aplica'
...
left join (
  ...
  on e.fecha = vce.fecha_agendamiento and e.origen = vce.origen
left join (
  select
    date(comprobantes.fecha_de_agendamiento) as fecha_agendamiento,
    case
  ...
  on e.fecha = vne.fecha_agendamiento and e.origen = vne.origen
group by e.fecha, e.origen, cc.cash_collected_total, fact.facturacion_total, vtas.ventas_totales, vce.ventas_cce, vne.ventas_ccne
