-- Ajuste de negocio:
-- para cash collected hay que contemplar Venta y Cobranza.
-- En Cobranza el monto util suele venir en cash_collected y no en cash_collected_total,
-- y muchas filas pueden tener producto_format vacio.
-- Facturacion y ventas siguen usando solo Venta.
...
comprobantes_cash AS (
  SELECT
    ...
    COALESCE(NULLIF(c.cash_collected_total, 0), c.cash_collected, 0)::numeric AS cash_monto
  FROM public.comprobantes c
  WHERE
    c.f_venta IS NOT NULL
