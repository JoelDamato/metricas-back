-- Ajuste solicitado:
-- cash_collected_real_mes se calcula por F.acreditacion (mes/año de acreditación)
-- sumando cash_collected_total, contemplando origen + estrategia.
...
cash_agenda_agg AS (
  ...
  FROM comprobantes_base
  WHERE acreditacion_anio = venta_anio AND acreditacion_mes = venta_mes
  GROUP BY venta_anio, venta_mes, origen_key, estrategia_key
),
cash_otros_agg AS (
  SELECT
