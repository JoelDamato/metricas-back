...
SELECT
  ...
  COALESCE(f.total_ventas_comp, 0::bigint) AS total_ventas,
  ...
  CASE
    WHEN COALESCE(b.total_aplica, 0::bigint) > 0
      THEN COALESCE(f.total_ventas_comp, 0::bigint)::numeric / b.total_aplica::numeric
    ELSE 0::numeric
  END AS tasa_cierre,
  CASE
    WHEN COALESCE(f.total_ventas_comp, 0::bigint) > 0
      THEN COALESCE(f.facturacion_total, 0::numeric) / f.total_ventas_comp::numeric
    ELSE 0::numeric
  END AS aov,
