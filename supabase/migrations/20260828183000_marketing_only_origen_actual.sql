create index if not exists leads_raw_ghlid_marketing_origin_idx
  on public.leads_raw ((lower(btrim(ghlid))))
...
with lead_source as (
  select
    l.*,
    nullif(btrim(l.origen_actual), '') as marketing_origin
  from public.leads_raw l
),
lead_origin_by_ghl as (
  select distinct on (lower(btrim(l.ghlid)))
    lower(btrim(l.ghlid)) as ghlid_key,
    l.marketing_origin
  from lead_source l
  where nullif(btrim(l.ghlid), '') is not null
    and l.marketing_origin is not null
  order by
    lower(btrim(l.ghlid)),
    l.last_edited_time desc nulls last,
    l.created_time desc nulls last
),
lead_norm as (
  select
    l.*,
    ...
    case
      when l.marketing_origin ilike 'Postulación MEG - %' then
        case
          when split_part(l.marketing_origin, ' - ', 2) ilike '%VSL%' then 'VSL'
          when split_part(l.marketing_origin, ' - ', 2) ilike '%ORG%' then 'ORG'
          when split_part(l.marketing_origin, ' - ', 2) ilike '%APSET%' then 'APSET'
          else split_part(l.marketing_origin, ' - ', 2)
        end
      else l.marketing_origin
    end as origen_norm
  from lead_source l
...
comp_source as (
  select
    c.*,
    nullif(btrim(lo.marketing_origin), '') as marketing_origin
  from public.comprobantes c
  left join lead_origin_by_ghl lo
    on lo.ghlid_key = lower(btrim(c.ghlid))
),
comp_norm as (
  select
    c.*,
    ...
    case
      when c.marketing_origin ilike 'Postulación MEG - %' then
        case
          when split_part(c.marketing_origin, ' - ', 2) ilike '%VSL%' then 'VSL'
          when split_part(c.marketing_origin, ' - ', 2) ilike '%ORG%' then 'ORG'
          when split_part(c.marketing_origin, ' - ', 2) ilike '%APSET%' then 'APSET'
          else split_part(c.marketing_origin, ' - ', 2)
        end
      else c.marketing_origin
    end as origen_norm
  from comp_source c
  where c.marketing_origin is not null
),
fechas as (
