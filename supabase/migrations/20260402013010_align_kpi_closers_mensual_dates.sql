...
with meses_base as (
  select distinct
    ...
    coalesce(l.closer, 'Sin closer') as closer
  from leads_raw l
  ...
  select distinct
    ...
    coalesce(c.creado_por, 'Sin closer') as closer
  from comprobantes c
...
cash_agg as (
  select
    ...
    extract(month from c.f_acreditacion)::integer as mes,
    coalesce(c.creado_por, 'Sin closer') as closer,
    sum(c.cash_collected) as cash_collected_mes
  from comprobantes c
  where upper(c.producto_format) <> 'CLUB'
  group by 1, 2, 3
),
    ...
    (
      ...
      from (
        select
          ...
          extract(month from c.f_venta)::integer as mes,
          coalesce(c.creado_por, 'Sin closer') as closer,
          sum(c.cash_collected) as cash_collected_mensual
        from comprobantes c
        where upper(c.producto_format) <> 'CLUB'
      ...
      where sub.closer = mb.closer
        and (
          (sub.anio = mb.anio and sub.mes < mb.mes and sub.mes > (mb.mes - 4))
          or
          (sub.anio = mb.anio - 1 and mb.mes <= 3 and sub.mes > (12 - (3 - mb.mes)))
        )
...
fact_3m as (
  select
    ...
    (
      ...
      where sub.closer = mb.closer
        and (
          (sub.anio = mb.anio and sub.mes < mb.mes and sub.mes >= (mb.mes - 3))
          or
          (sub.anio = mb.anio - 1 and mb.mes <= 3 and sub.mes >= (12 - (2 - mb.mes)))
        )
    ) as facturacion_3m
  from meses_base mb
)
