...
async function getStoredWorkflowRecords(month, requestedStatus) {
  ...
  const records = monthRows
    ...
    .map((row) => ({
      ...
      arcaIssuedAt: row.arca_response?.issuedAt || null,
      arcaServiceFrom: row.arca_response?.serviceFrom || null,
      arcaServiceTo: row.arca_response?.serviceTo || null,
      arcaCaeExpiration: row.arca_response?.caeExpiration || null,
      arcaPointOfSale: row.arca_response?.pointOfSale || null,
      arcaRecipientName: row.arca_response?.recipientName || null,
      arcaRecipientAddress: row.arca_response?.recipientAddress || null,
    ...
    const records = [...paymentRows, ...subscriptionRows]
      .map((record) => {
        ...
        return {
          ...
          arcaIssuedAt: workflow?.arca_response?.issuedAt || null,
          arcaServiceFrom: workflow?.arca_response?.serviceFrom || null,
          arcaServiceTo: workflow?.arca_response?.serviceTo || null,
          arcaCaeExpiration: workflow?.arca_response?.caeExpiration || null,
          arcaPointOfSale: workflow?.arca_response?.pointOfSale || null,
          arcaRecipientName: workflow?.arca_response?.recipientName || null,
          arcaRecipientAddress: workflow?.arca_response?.recipientAddress || null,
